...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_dir", type=str, default="/home/nas4_dataset/3D/NeRF/NeRF_Data/nerf_synthetic")
    ...
    scenes = ["chair", "drums", "hotdog", "lego", "materials", "mic", "ship"]
    # scenes = ["ficus"]

    for scene in scenes:
        data_dir = pathlib.Path(args.data_dir)
        data_dir = data_dir / scene
        cameras, images, points3d = read_model(data_dir, ".txt")
        ...
        with open(os.path.join(data_dir, 'depth_ranges.txt'), 'w') as f:
            for i in range(num_images):
                f.write('%f %f %f %f\n' % (depth_ranges[i+1][0], depth_ranges[i+1][1], depth_ranges[i+1][2], depth_ranges[i+1][3]))
        
        with open(os.path.join(data_dir, 'mvs_pairs.txt'), 'w') as f:
